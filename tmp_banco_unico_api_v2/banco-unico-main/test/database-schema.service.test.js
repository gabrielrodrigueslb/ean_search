...
test("ensureSchema cria extensoes, tabela e indices dentro de transacao", async () => {
  ...
  assert.ok(statements.some((statement) => statement.includes("CREATE TABLE IF NOT EXISTS departments")));
  assert.ok(statements.some((statement) => statement.includes("CREATE TABLE IF NOT EXISTS active_ingredients")));
  assert.ok(statements.some((statement) => statement.includes("embedding vector(512) NOT NULL")));
  assert.ok(statements.some((statement) => statement.includes("ALTER TABLE products ADD COLUMN IF NOT EXISTS department text")));
  assert.ok(statements.some((statement) => statement.includes("fk_products_department")));
